...
sigma = 25; %%%%% The Max Value of Noise
...
IMin=IMin0+sigma*randn(size(IMin0)); %%%%% Add Noise
PSNRIn = 20*log10(255/sqrt(mean((IMin(:)-IMin0(:)).^2))); %%%%% Peak Signal to Noise Ratio at Input
...
PSNROut = 20*log10(255/sqrt(mean((IoutDCT(:)-IMin0(:)).^2))); %%%%% Peak Signal to Noise Ratio at Output of DCT
figure;
...
%subplot(1,3,3); 
imshow(IoutDCT,[]); title(strcat(['Image Denoised by DCT dictionary, ',num2str(PSNROut),'dB']));
figure;
...
%subplot(1,3,2); imshow(IMin,[]); title(strcat(['Noisy image, ',num2str(PSNRIn),'dB']));
%subplot(1,3,3);
imshow(IoutGlobal,[]); title(strcat(['Image Denoised by Global Trained dictionary, ',num2str(PSNROut),'dB']));
figure;
...
figure; imshow(IMin0,[]); title('Original clean image');
figure; imshow(IMin,[]); title(strcat(['Noisy image, ',num2str(PSNRIn),'dB']));
figure; imshow(IoutAdaptive,[]); title(strcat(['Image Denoised by Adaptive dictionary, ',num2str(PSNROut),'dB']));
